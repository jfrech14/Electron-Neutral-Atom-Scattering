...
#define Numerov_H
  ...
  std::vector<double> ForwardNumerov(std::vector<std::vector<double> > &CrossSec, std::vector<std::vector<double> >& Vpot, double dh, int lmax, double scale, double eEnergy, double k, int iter)  
  {
      ...
      std::vector<long double> u (size,0.0);
      ...
      ddh12 = dh*dh/12.0; 
      i2 = (int) ((0.75*(size-1))*(2.5/3.0));
      r2 = Vpot[i2][0];
      i1 = (int) (((size-1))*(2.0/3.0));
      r1 = Vpot[i1][0];
      ...
      for (int l = 0; l <= lmax; l++ ) 
      {
          ...
          u[0] = 0;
          u[1] = dh/(1e50);
          u[2]=dh*dh*g(Vpot[1][1],eEnergy,Vpot[1][0],l)*u[1] +2*u[1];
